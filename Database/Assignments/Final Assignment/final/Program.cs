        ...
        static void Main(string[] args)
        {
            ...
            "VALUES ('student@hotmail.com', 'abc', '11-09-2001', 0, 1)";
                ...
                using (SqlConnection sqlConnection1 = new SqlConnection(conn))
                {
                    ...
                    updateQuery = "UPDATE TOP(1) [Netflix].[dbo].[User] SET user_email = 'hot#hotmail.com'";
                    before2 = DateTime.Now;
                    using (cmd)
                    {
                        cmd.CommandText = updateQuery;
...
/*                    using (sqlCommand)
                    ...
                    deleteQuery = "DELETE TOP(1) FROM [dbo].[User]";
                    before2 = DateTime.Now;
/*                    using (sqlCommand)
                    {
                        sqlCommand.CommandText = deleteQuery;
...
/*                    using (sqlCommand)
                ...
                using (var _context = new DatabaseContext())
                {
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        _context.Users.Add(user);
                        _context.SaveChanges();
                        /*                        string command = "INSERT INTO [User] (user_email, user_password, date_of_birth, is_blocked, subscription_id) " +
                                                        "VALUES ('student@hotmail.com', 'abc', '11-09-2001', 0, 1)";
                ...
                using (var dbContext = new DatabaseContext())
                {
                    string query = "SELECT TOP 10 * FROM [dbo].[User]";
                    var result = dbContext.Users.SqlQuery(query).First();
                }
                after3 = DateTime.Now;
                Console.WriteLine("Duration in miliseconds: {0}", (after3 - before3).TotalMilliseconds);
...
/*                    string query = "SELECT TOP 10 * FROM [dbo].[User]";
            ...
            /*for (int i = 0; i < 100; i++)
            {
                ...
                user.user_email = "student" + i + "@hotmail.com";
            ...
            /*for (int i = 0; i < 200; i++)
            {
                ...
                {
                    { "user_email", "student"+ 1000 + i + "@hotmail.com" },
            ...
            /*            for (int i = 0; i < 1000000; i++)
                        {
                            var document = new BsonDocument
                            {
                                { "user_email", "student"+ 100000 + i + "@hotmail.com" },
            ...
            Console.WriteLine("----------* Reading 1 row of data");
            before = DateTime.Now;
            var results = collection2.Find(_ => true).Limit(1);
            after = DateTime.Now;
            Console.WriteLine("Duration in miliseconds: {0}", (after - before).TotalMilliseconds);

            Console.Write("--------------* Reading 1,000 rows of data\n");
            before = DateTime.Now;
            results = collection2.Find(_ => true).Limit(1000);
            after = DateTime.Now;
            Console.WriteLine("Duration in miliseconds: {0}", (after - before).Milliseconds);
            Console.Write("--------------* Reading 100,000 rows of data\n");
            before = DateTime.Now;
            results = collection2.Find(x => true).Limit(100000);
            after = DateTime.Now;
            Console.WriteLine("Duration in miliseconds: {0}", (after - before).Milliseconds);
            Console.WriteLine("---------* Reading 1,000,000 rows of data");
            before = DateTime.Now;
            results = collection2.Find(x => true).Limit(1000000);
            after = DateTime.Now;
            Console.WriteLine("Duration in miliseconds: {0}", (after - before).Milliseconds);

            Console.WriteLine("---------* Updating 1 row of data");
            var search = Builders<UserMongo>.Filter.Regex("user_email", new BsonRegularExpression((new Regex("student", RegexOptions.IgnoreCase))));
            ...
            Console.WriteLine("------* Updating 1,000 rows of data");
            before = DateTime.Now;
            collection2.Find(_ => true).Limit(10).ForEachAsync(docs =>
            {
                collection2.UpdateOneAsync(Builders<UserMongo>.Filter.Regex("user_email", new BsonRegularExpression((new Regex("student", RegexOptions.IgnoreCase)))), Builders<UserMongo>.Update.Set("user_email", "hotUser@hotmail.com"));
            });
            after = DateTime.Now;
            Console.WriteLine("Duration in miliseconds: {0}", (after - before).Milliseconds);
            Console.WriteLine("------* Updating 100,000 rows of data");
            ...
            Console.WriteLine("-------* Deleting 1,000 rows of data");
            before = DateTime.Now;
            /*            collection2.Find(_ => true).Limit(2).ForEachAsync(
                                docs =>
                                {
